...
voici une [petite piqure de rappel](../../appendix/heap-vs-stack) pour vous rafraichir
la mémoire [^1].

## Pas de gargabe collector mais quand même du chocolat ?

Pour ne pas avoir de problème de mémoire, de nombreux langages (C#, Java, Python,
...
L'avantage est la simplicité d'utilisation. La contre partie, un impact sur les performances.
De plus, le *garbage collector* est géré automatiquement et pas forcément
complétement déterministe, on ne peut pas pas prévoir quand il s'exécutera.
...
allouée sur la stack est automatique, il n'y a pas de risque d'oublier d'appeler
...
*Note :* Dans cette section pointeur et référence seront considérés comme équivalent.*

*Rappel :* un [*dangling reference/pointer*](https://fr.wikipedia.org/wiki/Dangling_pointer)
est une référence vers un espace mémoire non valide. Dans les languages avec *garbage
...
Pour éviter ça, le compilateur de Rust vient encore à notre rescousse. A la compilation,
il vérifie la durée de vie des variables pour valider ou invalider les références.
Ainsi, si l'on écrit :
...
Le compilateur nous sort le message d'erreur : `y does not live long enough`.
En C ou C++, ce bug pourrait passer inaperçu jusqu’à l’exécution, alors qu’en
Rust il est détecté à la compilation.

### Le cas des retours de fonctions

Autre cas courant qui ne poserait pas problème à un compilateur C ou C++[^4] :
...
Jusque là, le language se débrouillait tout seul; cependant, parfois il faut l'aider.
...
Rust aurait attribué sa durée de vie à la référence de sortie (cf [*lifetime elision*](https://doc.rust-lang.org/nomicon/lifetime-elision.html)).
Mais ici, avec deux références en entrée, le language ne peut rien deviner.
...
elle commence par une apostrophe `'` puis et suivi d'un nom générique
(souvent `a, b, c, ect`).
...
retournée doit être compatible avec celle des deux entrées, donc elle est bornée
par la plus courte.
...
Rust à repris une forme de syntaxe que l'on retrouve dans de nombreux languages
...
et *lifetimes* pour garantir la sécurité mémoire sans garbage collector.
